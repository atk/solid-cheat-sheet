...
{
  localStorage.setItem("debug", ({}).DEBUG ?? "start*");
  // const { default: createDebugger } = await import("debug");
  // window._$DEBUG = createDebugger("start:client");
  window._$DEBUG = console.log;
  globalThis._$DEBUG(`({}).DEV = ${true}`);
  globalThis._$DEBUG(`({}).PROD = ${false}`);
  globalThis._$DEBUG(`({}).START_SSR = ${true}`);
  globalThis._$DEBUG(`({}).START_ISLANDS = ${false}`);
  globalThis._$DEBUG(`({}).START_ISLANDS_ROUTER = ${false}`);
  globalThis._$DEBUG(`({}).SSR = ${false}`);
  window.INSPECT = () => {
...
const _tmpl$$1 = /*#__PURE__*/template(`<a link></a>`, 2);
const Router = props => {
  const {
    source,
    url,
...
const _tmpl$ = /*#__PURE__*/template(`<div><div><p id="error-message"></p><button id="reset-errors">Clear errors and retry</button><pre></pre></div></div>`, 10);
  ...
  return createComponent(ErrorBoundary$1, {
    fallback: (e, reset) => {
      return createComponent(Show, {
        ...
        get fallback() {
          return createMemo(() => !!props.fallback)() && props.fallback(e, reset);
        },
        get children() {
...
function Scripts() {
  ...
  return [createComponent(voidFn, {}), isIslands , createComponent(NoHydration, {
    get children() {
      return isServer ;
    }
  }), isDev ];
...
export { A, Dynamic as D, NoHydration as N, Title as T, getNextMarker as a, createContext as b, createComponent as c, delegateEvents as d, onCleanup as e, createRenderEffect as f, getNextElement as g, className as h, insert as i, mergeProps as m, onMount as o, runHydrationEvents as r, setAttribute as s, template as t, useContext as u };
